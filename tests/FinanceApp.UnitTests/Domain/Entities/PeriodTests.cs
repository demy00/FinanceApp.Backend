...
#pragma warning restore CS8625 // Cannot convert null literal to non-nullable reference type.
    ...
    public void TotalSpent_ShouldCalculateCorrectly_WhenSingleCurrencyBillsArePresent()
    {
        ...
        bill1.AddBillItem(new BillItem("Milk", "1L", DefaultCategories.Groceries, new Money(3, "EUR"), new Quantity(2), userId));
        bill1.AddBillItem(new BillItem("Bread", "Whole grain", DefaultCategories.Groceries, new Money(2, "EUR"), new Quantity(1), userId));

        period.AddBill(bill1);

        Assert.Single(period.TotalSpent);
        Assert.Equivalent(new List<Money> { new Money(8, "EUR") }, period.TotalSpent);
    }
    ...
    public void TotalSpent_ShouldGroupByCurrency_WhenMultipleCurrenciesExist()
    {
        ...
        bill1.AddBillItem(new BillItem("Milk", "1L", DefaultCategories.Groceries, new Money(3, "USD"), new Quantity(2), userId));

        var bill2 = new Bill("Hotel", "Business trip", userId);
        bill2.AddBillItem(new BillItem("Hotel Stay", "3 Nights", DefaultCategories.Groceries, new Money(300, "EUR"), new Quantity(1), userId));

        period.AddBill(bill1);
        period.AddBill(bill2);

        Assert.Equal(2, period.TotalSpent.Count);
        Assert.Equal(new Money(6, "USD"), period.TotalSpent.Where(m => m.Currency == "USD").First());
        Assert.Equal(new Money(300, "EUR"), period.TotalSpent.Where(m => m.Currency == "EUR").First());
    }
